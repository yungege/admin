<?php
class Service_User_TeacherModel extends BasePageService {
    ...
    protected function __execute($req) {
        ...
        $options = [
            'limit' => self::PAGESIZE,
            'offset' => $offset,
            'sort' => ['createtime' => -1,'schoolinfo.schoolid' => -1],
        ];
        ...
        if(preg_match("/\w+/", $req['cid'])){
            $match['manageclassinfo.classid'] = trim($req['cid']);
        }

        if(preg_match("/\w+/", $req['sid'])){
            $match['schoolinfo.schoolid'] = trim($req['sid']);
        }
        ...
        if(!empty($req['sname'])){
            $match['schoolinfo.schoolname'] = ['$regex' => addslashes(trim($req['sname'])), '$options' => 'i'];
        }
