...
class Tree {
     ...
     * @return array
     */
    public function getTree($pid = 0, $pk = '_id', $pidFeldName = 'pid', $cateFieldName = 'name', $adds = '') {
        $num = 1;
     ...
     * @return array
     */
    public function getGridTree($pid = 0, $pk = '_id', $pidFeldName = 'pid', $cateFieldName = 'name', $adds = '') {
        $num = 1;
        $child = $this->getChild($pid, $pidFeldName);
        if(empty($child)) return $this->treeArr;
        $total = count($child);
        foreach ($child as $id => $value) {
            ...
            if($num == $total) $j = $this->icon[2];
            $spacer = $adds ? $adds . $j : '';
            $value['new_name'] = $spacer.' '.$value[$cateFieldName];
            $value['new_sort'] = $spacer.' '.$value['sort'];
     ...
     */
    public function getTreeArray($pk = '_id', $pidFeldName = 'pid', $child = '_child', $root = 0) {
        //基于主键的数组引用
        $refer = [];
